...
var BlogSize = sassVars.bar_size_short * sassVars.columns + sassVars.base_margin * (sassVars.columns - 1);
...
exports.default = {
  ...
  /**
   * Horizontal Blogs configuration
   * @type {Object}
   */
  BlogsHorz: {
    saturation: {
...
var _BlogHandler = __webpack_require__(13);

var _BlogHandler2 = _interopRequireDefault(_BlogHandler);
...
var Colorpicker = function () {
  ...
  function Colorpicker(element, options) {
     ...
     * @type {BlogHandler}
     */
    this.BlogHandler = new _BlogHandler2.default(this);
    /**
     * @type {PopupHandler}
     */
  ...
  _createClass(Colorpicker, [{
    ...
    value: function init() {
      ...
      // Init Blogs and popup
      this.BlogHandler.bind();
...
/**
 * Class that handles all configured Blogs on mouse or touch events.
 * @ignore
 */
var BlogHandler = function () {
   ...
   * @param {Colorpicker} colorpicker
   */
  function BlogHandler(colorpicker) {
    _classCallCheck(this, BlogHandler);
  ...
  /**
   * This function is called every time a Blog guide is moved
   * The scope of "this" is the BlogHandler object.
   *
   * @param {int} top
   * @param {int} left
   */
  ...
  _createClass(BlogHandler, [{
    ...
    value: function defaultOnMove(top, left) {
      ...
      Blog.guideStyle.left = left + 'px';
      Blog.guideStyle.top = top + 'px';

      // Adjust the color
      if (Blog.callLeft) {
        color[Blog.callLeft](left / Blog.maxLeft);
      }
      if (Blog.callTop) {
        color[Blog.callTop](top / Blog.maxTop);
      }
    ...
    /**
     * Binds the colorpicker Blogs to the mouse/touch events
     */

  }, {
    key: 'bind',
    value: function bind() {
      var Blogs = this.colorpicker.options.horizontal ? this.colorpicker.options.BlogsHorz : this.colorpicker.options.Blogs;

      var BlogClasses = [];

      for (var BlogName in Blogs) {
        if (!Blogs.hasOwnProperty(BlogName)) {
          continue;
        }

        BlogClasses.push(Blogs[BlogName].selector);
      }

      this.colorpicker.picker.find(BlogClasses.join(', ')).on('mousedown.colorpicker touchstart.colorpicker', _jquery2.default.proxy(this.pressed, this));
    }
    ...
    value: function pressed(e) {
      ...
      // detect the Blog and set the limits and callbacks
      ...
      for (var BlogName in Blogs) {
        ...
        if (zone.is(Blog.selector)) {
          this.currentBlog = _jquery2.default.extend({}, Blog, { name: BlogName });
          break;
        } else if (Blog.childSelector !== undefined && zone.is(Blog.childSelector)) {
          this.currentBlog = _jquery2.default.extend({}, Blog, { name: BlogName });
          zone = zone.parent(); // zone.parents(Blog.selector).first() ?
          break;
        }
      }
      ...
      if (this.currentBlog === null || guide === null) {
        return;
      }
      ...
      this.currentBlog.guideStyle = guide.style;
      this.currentBlog.left = e.pageX - offset.left;
      this.currentBlog.top = e.pageY - offset.top;
      this.mousePointer = {
        left: e.pageX,
        top: e.pageY
      };
    ...
    value: function moved(e) {
      ...
      var left = Math.max(0, Math.min(this.currentBlog.maxLeft, this.currentBlog.left + ((e.pageX || this.mousePointer.left) - this.mousePointer.left)));

      var top = Math.max(0, Math.min(this.currentBlog.maxTop, this.currentBlog.top + ((e.pageY || this.mousePointer.top) - this.mousePointer.top)));
  ...
  _createClass(PickerHandler, [{
    ...
    value: function update() {
      ...
      var vertical = this.options.horizontal !== true,
          Blog = vertical ? this.options.Blogs : this.options.BlogsHorz;
      ...
      // Set guides position
      if (hueGuide.length) {
        hueGuide.css(vertical ? 'top' : 'left', (vertical ? Blog.hue.maxTop : Blog.hue.maxLeft) * (1 - hsva.h));
      }
      if (alphaGuide.length) {
        alphaGuide.css(vertical ? 'top' : 'left', (vertical ? Blog.alpha.maxTop : Blog.alpha.maxLeft) * (1 - hsva.a));
      }
      if (saturationGuide.length) {
        saturationGuide.css({
          'top': Blog.saturation.maxTop - hsva.v * Blog.saturation.maxTop,
          'left': hsva.s * Blog.saturation.maxLeft
        });
      }
